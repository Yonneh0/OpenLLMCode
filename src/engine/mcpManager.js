...
export async function discoverLocalServers(projectRoot) {
    const discovered = [];
    if (!projectRoot)
    ...
    try {
        ...
        if (config.transport === 'stdio' && config.command) {
            const transport = new StdioClientTransport({
                ...
                args: config.args || [],
                env: (config.env !== undefined) ? Object.fromEntries(Object.entries({ ...process.env, ...config.env }).filter(([_, v]) => v != null)) : undefined,
            });
            client = new Client({ name: `OpenLLMCode-client`, version: '0.1.0' }, { capabilities: {} });
 ...
 */
async function unregisterServerToolsFromRegistry(serverId) {
...
function existingToolsForServer(serverId) {
 ...
 */
export function getMCPToolsForSystemPrompt() {
    const tools = [];
        ...
        for (const tool of server.tools) {
            ...
            try {
                toolRegistry.registerTool({
                    name: mcpToolName, // ToolType will accept unknown types at runtime — MCP tools use "id:name" format
                    description: `[${server.name}] ${tool.description}`,
                    parameters,
 ...
 */
export async function unregisterMCPToolsFromRegistry() {
    if (!mcpToolsRegistered)
        return;
    const toolRegistry = await import('./toolRegistry.js');
    for (const server of registeredServers.values()) {
 ...
 */
export async function callMCPTool(serverToolName, params) {
...
export async function saveMCPConfigs(projectRoot) {
    const pathModule = await import('path');
    const configPath = pathModule.join(projectRoot, '.openllmcode-mcp');
...
export async function loadMCPConfigs(projectRoot) {
    const pathModule = await import('path');
    const configPath = pathModule.join(projectRoot, '.openllmcode-mcp');
    try {
        const fs = await import('fs');
 ...
 */
export async function autoConnectServers(projectRoot) {
...
export function getMCPHealthSummary() {
    let total = 0;
    let connected = 0;
    let disconnected = 0;
    let error = 0;
