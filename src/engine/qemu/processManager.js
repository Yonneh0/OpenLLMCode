...
class QEMUProcessManager {
    ...
    buildArgs(config) {
        ...
        const arch = config.architecture; // Cast from any to fix TS indexing errors on Records (line 39)
        ...
        if (config.cpuTopology?.sockets || config.cpuTopology?.cores || config.cpuTopology?.threads) {
            ...
            for (const [key, val] of Object.entries(config.cpuTopology)) {
                if (val != null && key !== 'maxcpus')
                    smpParts.push(`${key}=${val}`);
            }
        ...
        config.diskImages?.forEach((disk) => {
            const driveArgs = [
        ...
        config.networkDevices?.forEach((nic) => {
            ...
            if (nic.macAddress)
                netdevArgs += `,macaddr=${nic.macAddress}`; // Per NIC/macaddr docs
            ...
            args.push(`-device ${nicModels[arch] || 'virtio-net-pci'},netdev=${String(nic.id)}`); // eslint-disable-line @typescript-eslint/no-explicit-any — config.architecture is any type at runtime (per QEMU docs)
        });
    ...
    async createVM(config) {
        ...
        proc.stdout?.on('data', (d) => {
            ...
            try { /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
                global.mainWindow?.webContents.send('qemu-output', { vmId: config.id, data: d.toString(), type: 'stdout' });
            }
            catch { }
        });
        proc.stderr?.on('data', (d) => {
            ...
            try { /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
                global.mainWindow?.webContents.send('qemu-output', { vmId: config.id, data: d.toString(), type: 'stderr' });
            }
            catch { }
        });
        return instance;
    ...
    async executeQMPCommand(vmId, command, args) {
    ...
    async deleteVM(vmId) {
        ...
        try { /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
            require.fs.unlinkSync(vm.monSocket.address);
        }
        catch { }
    }
    ...
    async hotplugCPU(vmId, socketId) {
    ...
    async addMemory(vmId, sizeBytes) {
        ...
        const vm = this.instances.get(vmId);
        if (vm)
            vm.ramBytes += sizeBytes; // Update instance state after successful RAM addition
    }
    async queryBlockDevices(vmId) {
        return this.executeQMPCommand(vmId, 'query-block');
    }
    async createSnapshot(vmId, driveId) {
        ...
        const snapshotId = result.id; // Returns job ID from background jobs section of QMP spec  
    ...
    async createDiskImage(format, sizeMB, path) {
        const result = await this.executeCommand(`qemu-img create -f ${format} "${path}" ${sizeMB}M`);
        if (result.exitCode !== 0)
            throw new Error(`Failed to create disk image: ${result.stderr}`);
    }
    async convertDiskImage(srcFormat, dstFormat, srcPath, dstPath) {
        const result = await this.executeCommand(`qemu-img convert -f ${srcFormat} -O ${dstFormat} "${srcPath}" "${dstPath}"`);
        if (result.exitCode !== 0)
            throw new Error(`Failed to convert disk image: ${result.stderr}`);
    }
