...
// ─── Tool registry state ──────────────
let tools = new Map(BUILTIN_TOOLS.map(t => [t.name, t]));
...
export function registerTool(def) {
    ...
    if (existing && !isMCPTOOL(def.name))
        return;
...
// ─── Approval rules ──────────────
let approvalRules = { categories: {} };
// Load approval rules from .openllmcode-rules file in project root
export function loadApprovalRules(projectRoot) {
...
export function saveApprovalRules(projectRoot) {
    const rulesPath = path.join(projectRoot, '.openllmcode-rules');
...
export function requiresApproval(toolType, input) {
...
export function addAlwaysAllowRule(category, pattern, reason) {
    const cat = approvalRules.categories[category] || { default: 'require_approval' };
    if (!cat.allow_patterns)
        cat.allow_patterns = [];
    // Avoid duplicates
...
function getCategoryForTool(toolType) {
    switch (toolType) {
...
function matchGlob(filePath, pattern) {
    ...
    const regex = new RegExp('^' + pattern.replace(/\./g, '\\.').replace(/\*\*/g, '___DOUBLESTAR___').replace(/\*/g, '[^/]*').replace(/___DOUBLESTAR___/g, '.*') + '$', 'i');
...
export async function executeTool(toolType, input) {
    const api = getApi();
    if (!api)
        ...
        switch (toolType) {
            ...
            case 'terminal_run_command': {
                ...
                try {
                    ...
                    const cleanup = api.terminal.onData((data) => {
